...
from tools.model_provider_shadow import (  # noqa: E402
...
class RuntimePolicy:
    ...
    def load(cls, path: Path) -> "RuntimePolicy":
        ...
        if (
            ...
            or value["policy_id"] != "r04c-connected-worker-runtime-v1"
        ...
        for name in (
            ...
            digests[name] = raw
        expected_paths = {
            "worker_ipc_extension_sha256": REPOSITORY_ROOT
            / "provenance"
            / "model-worker-ipc-extension-v1.json",
            "shadow_profile_sha256": REPOSITORY_ROOT
            / "provenance"
            / "model-provider-connected-shadow-v2.json",
            "shadow_tool_sha256": REPOSITORY_ROOT
            / "tools"
            / "model_provider_shadow.py",
        }
        if any(_sha256_file(expected_paths[name]) != digest for name, digest in digests.items()):
