...
def _run(*args: str, cwd: Path | None = None) -> subprocess.CompletedProcess[bytes]:
    ...
    if result.returncode != 0:
        raise AssertionError(f"command failed without exposing subprocess output: {args[0]}")
    return result
...
def _registry(tmp_path: Path, *, revoked_at: str | None = None) -> tuple[Path, str, str, Path]:
    ...
    if key_path.stat().st_mode & 0o077:
        raise AssertionError("temporary test key permissions are too broad")
...
class S34AttestationAnchorTests(unittest.TestCase):
    def test_signature_binds_exact_receipt_and_waits_for_anchor(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            policy, registry, receipt, attestation = _signed(Path(directory))
            result = verify_attestation(
                policy, registry, receipt, attestation, now=VERIFY_AT
            )
            self.assertEqual(result.status, "WAIT_ANCHOR")
            self.assertTrue(result.signature_verified)
            self.assertFalse(result.anchor_verified)
            self.assertFalse(result.grants_authority)
            self.assertIs(attestation["private_key_present"], False)
            self.assertNotIn("PRIVATE KEY", json.dumps(attestation))

            with self.assertRaisesRegex(AttestationError, "binding mismatch"):
                verify_attestation(
                    policy, registry, receipt + b" ", attestation, now=VERIFY_AT
                )

    def test_frozen_public_attestation_verifies_offline_and_is_anchored(self) -> None:
        policy = AttestationPolicy(POLICY_PATH, expected_sha256=POLICY_SHA256)
        ...
        self.assertEqual(anchored.status, "VERIFIED_AND_ANCHORED")
        self.assertTrue(anchored.signature_verified)
        self.assertTrue(anchored.anchor_verified)
        self.assertFalse(anchored.grants_authority)

    def test_signature_substitution_and_expiry_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            policy, registry, receipt, attestation = _signed(Path(directory))
            ...
            with self.assertRaisesRegex(AttestationError, "not currently valid"):
                verify_attestation(
                    policy, registry, receipt, attestation,
                    now="2026-08-02T00:40:00Z",
                )

    def test_revoked_and_substituted_keys_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            revoked_path, revoked_sha, key_id, _ = _registry(
                root / "revoked", revoked_at="2026-07-20T00:00:00Z"
            )
            revoked = PublicKeyRegistry(revoked_path, expected_sha256=revoked_sha)
            with self.assertRaisesRegex(AttestationError, "inactive or revoked"):
                revoked.key(key_id, at=datetime(2026, 7, 19, 1, tzinfo=timezone.utc))

            first_path, first_sha, _, _ = _registry(root / "first")
            value = json.loads(first_path.read_text(encoding="ascii"))
            second_path, _, _, _ = _registry(root / "second")
            ...
            with self.assertRaisesRegex(AttestationError, "material digest mismatch"):
                PublicKeyRegistry(
                    substituted, expected_sha256=hashlib.sha256(raw).hexdigest()
                )
            self.assertNotEqual(first_sha, hashlib.sha256(raw).hexdigest())

    def test_invalid_and_exact_git_anchor_states(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            ...
            result = verify_attestation(
                policy, registry, receipt, attestation, now=VERIFY_AT,
                anchor=invalid, repository_root=root,
            )
            self.assertEqual(result.status, "REJECTED_ANCHOR")

            repository = root / "anchor-repository"
            ...
            self.assertEqual(result.status, "VERIFIED_AND_ANCHORED")
            self.assertTrue(result.signature_verified)
            self.assertTrue(result.anchor_verified)
            self.assertFalse(result.grants_authority)

    def test_private_key_inside_current_repository_is_refused(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            registry = PublicKeyRegistry(path, expected_sha256=digest)
            with mock.patch("tools.attestation.Path.cwd", return_value=repository):
                with self.assertRaisesRegex(AttestationError, "outside repository"):
                    sign_receipt(
                        policy, registry, RECEIPT_PATH.read_bytes(), key_id=key_id,
                        private_key_path=private_key, signed_at=SIGNED_AT,
                        expires_at=EXPIRES_AT,
                    )

    def test_policy_scope_and_subprocess_non_disclosure_guards(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            policy = AttestationPolicy(POLICY_PATH, expected_sha256=POLICY_SHA256)
            path, digest, key_id, private_key = _registry(Path(directory))
            registry = PublicKeyRegistry(path, expected_sha256=digest)
            outside = {
            ...
            with self.assertRaisesRegex(AttestationError, "outside selected"):
                sign_receipt(
        ...
        self.assertNotIn("private_key_path.read", source)
