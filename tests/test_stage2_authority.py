...
class Stage2AuthorityTests(unittest.TestCase):
    ...
    def test_market_corrupt_checkpoint_worker_lease_is_exact_and_receipt_bound(self) -> None:
        receipt = load(
            INTEGRATION_RECEIPTS / "s2-market-corrupt-checkpoint-authority.json"
        )
        envelope = load(STAGES / "s2-market-corrupt-checkpoint" / "stage-envelope.json")
        lease = load(STAGES / "s2-market-corrupt-checkpoint" / "ownership-lease.json")

        self.assertEqual(receipt["schema_id"], "IntegrationReceipt")
        self.assertEqual(receipt["integrity"]["payload_sha256"], payload_sha256(receipt))
        self.assertEqual(
            receipt["payload"]["head_sha"],
            "d187111a4dd360a2e7e76cfbfc8525c92d98d423",
        )
        audit = receipt["payload"]["audit_results"]
        self.assertFalse(audit["declares_market_pre_soak_green"])
        self.assertFalse(audit["public_runtime_checkpoint_payload_in_public_repo"])
        self.assertFalse(audit["silent_repair_authorized"])
        self.assertFalse(audit["backup_restore_scope_included"])
        self.assertEqual(envelope["public_authority_sha"], receipt["payload"]["head_sha"])
        self.assertEqual(envelope["public_authority_ci"], "29613589742")
        self.assertEqual(
        ...
        self.assertFalse(envelope["push_authority"])
        self.assertIn(
            "validator-accepts-undetected-corruption-new-work-after-detection-completion-silent-repair-non-RECOVERY_REQUIRED-state-or-runtime-public-copy",
            envelope["stop_conditions"],
        )
